...
/** Enums to be specified for Runtime monitoring \c enableMonitoring: / \c disableMonitoring: / \c isMonitoringEnabled:. */
typedef NS_ENUM(NSInteger, CAMDOMonitoringType) {
    NETWORK = 0,
    WEBVIEW = 1,
};
 ...
 */
+ (BOOL) isMonitoringEnabled:(CAMDOMonitoringType)type;
 ...
 *
 * Recommended transactionContextTarget types (\c id): \c NSMutableURLRequest, \c NSURLRequest (immutable), \c NSURL, or \c NSString (URL string parsed with \c +[NSURL URLWithString:]).
 */
 ...
 * @param transactionContextTarget Target (\c id): \c NSMutableURLRequest, \c NSURLRequest (immutable), \c NSURL, or \c NSString (URL string parsed with \c +[NSURL URLWithString:]).
 ...
 *
 * @param target Target (\c id): \c NSMutableURLRequest, \c NSURL, \c NSString (URL string), or \c dispatch_queue_t.
 ...
 *
 */
+ (void) stopNetworkTransactionWithName:(NSString *) transactionName completionHandler:(void(^)(BOOL completed,NSError *error))completionBlock;
...
+ (void) stopNetworkTransactionWithName:(NSString *) transactionName failure:(NSString *) failure completionHandler:(void(^)(BOOL completed,NSError *error)) completionBlock;
